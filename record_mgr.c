...
/*
 * This function is created for Nested Loop Join functionality
 * This function returns the Record from the relation/ Table
 */
char* getRecordDataForNLJ (RM_TableData *rel, RID id, Record *record)
{
	...
	if(id.page > 0 && id.page <=  totalPages)
	{
		...
		temp = deserializeRecordKey(record_data,rel->schema);
...
/*
 * This function is used along with the startScan function,
 * call to this method returns the tuples required for SCANNING
 * It is used for NESTED LOOP JOIN functionanlity
 */
RC nextNLJ (RM_ScanHandle *scan, Record *record)
{
	Value *result;
	RID rid;
